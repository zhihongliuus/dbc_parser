...
TEST_F(DbcParserTest, ParseNewSymbols) {
  ...
  EXPECT_TRUE(std::find(db->new_symbols().begin(), db->new_symbols().end(),
                        "CM_") != db->new_symbols().end());
  EXPECT_TRUE(std::find(db->new_symbols().begin(), db->new_symbols().end(),
                        "BA_DEF_") != db->new_symbols().end());
}
...
TEST_F(DbcParserTest, ParseAttributeDefaults) {
  ...
  EXPECT_EQ("",
            std::get<std::string>(db->attribute_defaults().at("SignalType")));
  EXPECT_EQ("100", std::get<std::string>(
                       db->attribute_defaults().at("GenMsgCycleTime")));
  EXPECT_EQ("ECU",
            std::get<std::string>(db->attribute_defaults().at("NodeType")));
  EXPECT_EQ("CAN",
            std::get<std::string>(db->attribute_defaults().at("NetworkType")));
}
...
TEST_F(DbcParserTest, ParseAttributeValues) {
  ...
  EXPECT_EQ("CAN Powertrain",
            std::get<std::string>(db->global_attributes().at("NetworkType")));

  // Message attributes
  auto engine_msg = db->get_message(100);
  ASSERT_TRUE(engine_msg);
  EXPECT_EQ("100", std::get<std::string>(
                       engine_msg->attributes().at("GenMsgCycleTime")));

  auto trans_msg = db->get_message(200);
  ASSERT_TRUE(trans_msg);
  EXPECT_EQ("200", std::get<std::string>(
                       trans_msg->attributes().at("GenMsgCycleTime")));
  ...
  ASSERT_TRUE(ecu2);
  EXPECT_EQ("Transmission",
            std::get<std::string>(ecu2->attributes().at("NodeType")));
}
...
TEST_F(DbcParserTest, ParseSignalTypeRefs) {
  ...
  EXPECT_EQ("EngineSpeedType",
            db->signal_type_refs().at(100).at("EngineSpeed"));
}
...
TEST_F(DbcParserTest, DecodeFrames) {
  ...
  std::vector<uint8_t> engine_data = {0xE8, 0x03, 0x50, 0x40, 0x00,
                                      0x00, 0x00, 0x00};  // 1000 rpm, 80C, 64%
...
TEST_F(DbcParserTest, ParseSimpleString) {
  const std::string simple_dbc = R"(
VERSION "1.0"
...
BO_ 100 EngineData: 8 Node1
 SG_ EngineSpeed : 0|16@1+ (0.1,0) [0|6500] "rpm" Node2
 SG_ EngineTemp : 16|8@1+ (1,-40) [-40|215] "degC" Node2

BO_ 200 TransmissionData: 8 Node1
 SG_ GearPosition : 0|4@1+ (1,0) [0|8] "" Node2
 SG_ TransmissionTemp : 8|8@1+ (1,-40) [-40|215] "degC" Node3
...
CM_ SG_ 100 EngineSpeed "Engine rotational speed";
  ...
  auto parser = std::make_unique<DbcParser>();
  auto db = parser->parse_string(simple_dbc, options_);
  ASSERT_TRUE(db);
  ASSERT_TRUE(db->version());
  EXPECT_EQ("1.0", db->version()->version);
}

TEST_F(DbcParserTest, ParseVersionOnly) {
  const std::string version_only = R"(VERSION "1.0")";

  auto parser = std::make_unique<DbcParser>();
  auto db = parser->parse_string(version_only, options_);
  ASSERT_TRUE(db);
  ASSERT_TRUE(db->version());
  EXPECT_EQ("1.0", db->version()->version);
}

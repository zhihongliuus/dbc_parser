...
TEST(NodesParserTest, ParsesSingleNode) {
  ...
  EXPECT_EQ(result->nodes[0].name, "ECU1");
}
...
TEST(NodesParserTest, ParsesMultipleNodes) {
  ...
  auto result = NodesParser::Parse(input);
  ASSERT_TRUE(result.has_value());
  ASSERT_EQ(result->nodes.size(), 4);
  EXPECT_EQ(result->nodes[0].name, "ECU1");
  EXPECT_EQ(result->nodes[1].name, "ECU2");
  EXPECT_EQ(result->nodes[2].name, "Gateway");
  EXPECT_EQ(result->nodes[3].name, "Vector_XXX");
}
...
TEST(NodesParserTest, HandlesNodesWithSpecialChars) {
  ...
  ASSERT_TRUE(result.has_value());
  ASSERT_EQ(result->nodes.size(), 3);
  EXPECT_EQ(result->nodes[0].name, "ECU_123");
  EXPECT_EQ(result->nodes[1].name, "Node-With-Dash");
  EXPECT_EQ(result->nodes[2].name, "Node_With_Underscore");
}
